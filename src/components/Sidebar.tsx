        ...
        <div className="fixed inset-0 z-50 flex md:inset-y-0 md:left-0 md:right-auto md:w-96">
          ...
          <div className="relative bg-white w-full sm:w-80 md:w-96 h-full shadow-lg p-5 transform transition-transform duration-300">
            ...
            <div className="flex gap-5 flex-col">
              {[...new Set(cars.map((car) => car.category))].map(
                (category, index) => (
                  <label key={index} className="flex items-center gap-2">
                    ...
                    <span className="text-gray-500">
                      (
                      {
                        filteredCars.filter((c) => c.category === category)
                          .length
                      }
                      )
                    </span>
            ...
            <div className="flex gap-5 flex-col">
              {[...new Set(cars.map((car) => car.seats))].map(
                (seats, index) => (
                  <label key={index} className="flex items-center gap-2">
                    <input
                      type="checkbox"
                    ...
                    <span className="text-gray-500">
                      ({filteredCars.filter((c) => c.seats === seats).length})
                    </span>
                  </label>
                ),
              )}
